...
hostname = 'redis-11912.c267.us-east-1-4.ec2.cloud.redislabs.com'
portnumber= 11912
password='7MLvakLqfNl6sYXyGh1NFy7hPmSIshhi'
...
def retrieve_data(name):
    ...
    retrieve_df[['Name','Role','Emp_Id']] = retrieve_df['name_role'].apply(lambda x: x.split('@')).apply(pd.Series)
    return retrieve_df[['Name','Role','Emp_Id','facial_features']]
...
class RealTimePred:
    ...
    def saveLogs_redis(self):
        ...
        #step-2: drop the duplicate info (distinct name)
        ...
        if len(encoded_data) > 0:
            r.lpush('ArcEmployeeattendance:logs',*encoded_data)
    ...
    def face_prediction(self,test_image,dataframe,feature_column,name_role =['Name','Role'],thresh=0.5):
        ...
        # step-1: take the test imageand apply to insight face
...
class RegistrationForm:
    ...
    def save_data_in_redis_db(self,name,role,Emp_Id):

        if name is not None:
            if name.strip() != '':
                 key = f'{name}@{role}@{Emp_Id}'
            else:
                 return 'name_false'
        ...
        # step-4: save thiss into redis db
        # redis hashes
        print(r.ping)
        r.hset(name='ArcEmployees:register',key=key,value=x_mean_bytes)
        print(r.keys)
